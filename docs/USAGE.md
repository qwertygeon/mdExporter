...
- **출력 경로 충돌**: 서로 다른 입력이 같은 출력 파일로 향하면 **변환을 시작하기 전에 전체를 중단**하고 충돌한 경로를 알린다 — 앞서 만든 산출물이 조용히 덮어써지지 않게 하기 위해서다. 대소문자를 구분하지 않는 파일시스템(macOS·Windows)에서 이름이 대소문자만 다른 입력(`Guide.md`·`guide.md`)도 같은 파일이 되므로 함께 감지한다. 이 경우 `--out-dir` 로 출력을 나누거나 파일명을 바꿔야 한다.
