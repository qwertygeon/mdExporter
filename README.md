...
- **이미지가 산출물에 담긴다**: 문서가 참조하는 로컬 이미지를 HTML·PDF 안에 인라인하므로, 산출물만 옮겨도·PDF 로 뽑아도 이미지가 그대로 보인다.
- **mermaid 다이어그램**: ` ```mermaid ` 로 적은 순서도·시퀀스 다이어그램 등을 변환 시점에 그림(SVG)으로 그려 본문에 담는다. 산출물에 스크립트가 붙지 않아 HTML 과 PDF 가 같은 그림을 공유한다.
...
- **Node.js 18 이상**
- **PDF 생성 시, 그리고 mermaid 다이어그램을 그릴 때**: Playwright 용 Chromium 브라우저. 최초 1회 설치가 필요하다.
  ```bash
  npx playwright install chromium
  ```
  (HTML 만 생성하고 mermaid 를 쓰지 않으면 브라우저가 필요 없다. 브라우저 없이 mermaid 문서를 변환하면 경고와 함께 다이어그램만 코드블록으로 남고 변환은 계속된다.)
...
| `--toc` | 자동 목차 생성 | 꺼짐 |
| `--toc-depth <n>` | 목차 포함 깊이(h2~h{n}, 2 이상의 정수) | 3 |
| `--cover` | 표지 페이지 추가 | 꺼짐 |
| `--date <date>` | 표지 날짜(지정 시만 표시) | 없음 |
| `--header` | PDF 헤더(문서 제목) | 꺼짐 |
| `--footer` | PDF 푸터(페이지 번호) | 꺼짐 |
| `--no-embed-images` | 로컬 이미지를 인라인하지 않고 원본 경로를 유지(산출물이 작아지는 대신 PDF 에서 이미지가 보이지 않음) | 인라인함 |
| `--no-mermaid` | ` ```mermaid ` 펜스를 그림으로 그리지 않고 코드블록으로 남김 | 그림으로 그림 |
